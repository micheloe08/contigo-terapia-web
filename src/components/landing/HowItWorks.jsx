      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          {STEPS.map((s) => (
            <div
              key={s.num}
              className="bg-white border border-slate-200 rounded-2xl p-8 text-center hover:shadow-sm transition-shadow"
            >
              <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center mx-auto mb-5 text-white text-sm font-medium">
                {s.num}
              </div>
              <div className="w-14 h-14 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4">
                <i className={`ti ${s.icon} text-blue-600 text-2xl`} />
              </div>
              <p className="font-medium text-slate-900 text-lg mb-3">
                {s.title}
              </p>
